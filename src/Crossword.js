...
// // Function to fill the remaining empty cells with random characters
// const fillEmptyCellsWithRandomLetters = (grid) => {
//   const filledGrid = grid.map((row) =>
//     row.map((cell) => (cell === null ? generateRandomLetter() : cell))
//   );
//   return filledGrid;
// };
...
//   useEffect(() => {
//     const initialGrid = placeWords(words, gridSize); // Place words
//     const gridWithRandomLetters = fillEmptyCellsWithRandomLetters(initialGrid); // Fill empty cells with random letters
//     setGrid(gridWithRandomLetters);
//   }, []);
...
//In this below: let's take [0,1] where 0 is the row and 1 is the column
const directions = [
  [0, 1], //Right direction
  [0, -1], //Left
  [1, 0], //down
  [-1, 0], //down to up
  [1, 1], //diagonal top to bottom
  [1, -1], //diafonal right to bottom left
  [-1, 1], //diagonal up right side
  [-1, -1], //diagonal up to left side
];
